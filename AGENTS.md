...
- [x] **P58 审计加固:AI 确认点超时 + 任务 abort 兜底 + 前端清理(已完成,`cargo build`+`cargo test` 86 项+`npm run build` 通过,未提交)**:三项审计(命令注入/前端安全/并发资源)定位的发布阻断项——AI 确认点 `ctl_rx.recv().await` 无超时,用户不响应时任务永久阻塞且 busy 卡死、删会话无法解套。修复:① ai_job.rs 新增 `recv_confirm`(内部 `tokio::time::timeout(Duration::from_secs(CONFIRM_TIMEOUT_SECS=120))`)包裹计划级与逐条命令两级确认,超时/通道关闭/取消一律**中止任务而非静默执行未批命令**(安全默认),超时发 `Done{等待确认超时,本次计划已中止}`;② core.rs `AiSlot` 增 `task: Mutex<Option<JoinHandle<()>>>` 保存 spawned AI 任务句柄,`delete_session`→`AiManager::remove` 时 `abort()`,避免删除会话后僵尸任务持续占用槽/busy;任务正常结束自动清空句柄。③ 前端 App.svelte:启动 `listSessions`/`getAiConfig` 两个 `.then` 补 `.catch`(防 unhandled rejection),删除只写不读的死状态 `aiState`(亦是 Svelte5 `state_referenced_locally` 告警之源的清理项)。已确认安全/设计使然的项(不改):AI `cmd` 原样进 bash 属 Agent 核心特性(防护=危险判定+整份计划确认门)、SFTP 无路径沙箱(与 root 运维工具威胁模型一致)、crypto.rs unsafe(DPAPI FFI 封闭)、新凭据 DPAPI 加密、命令注入仅 cwd 需转义(已正确转义)、前端无 `{@html}`/DOM XSS。
