...
- [x] **P34 Windows 监控 + Agent + SFTP(由 P32 三期规划落地前两期核心,已完成 develop 侧代码,`cargo build`+`cargo test` 61 项+`npm run build` 通过;**无 Windows 真机,`cargo tauri build` 与真机冒烟待 Windows 测试机就绪后补跑**)**。
  ...
  2. **验证**:`cargo build` 零错误(原 P34-1 遗留两处编译错修复:`connect` 里 `sessions.insert(name,..)` 后 `get_mut(&name)` 借用已移动 → 改 `name.clone()`;`kind_of` 尾部表达式临时 MutexGuard 借用 → 改 `let guard = …; guard.kind` 先绑再取)、`cargo test` **61 项全过**(58 + P34-1 监控 1 + P34-2 Agent 2)、`npm run build` 通过。**~~待办:Windows 测试机真机冒烟~~(2026-08-29 用户确认无法测试,该项关闭**;单测已覆盖 PowerShell 编码组装/解析,Windows 会话能力未经真机验证属已知限制——若将来有测试机,按原清单补:PowerShell 监控数字、Agent 命令执行与 `$PWD` 回传、文件面板 `C:\` 路径导航/上传/下载/预览保存、rdp 占位)。
...
- [x] **P71 Docker 交互终端(补 P54 首版缺口,已完成,`cargo build` 零警告 + `cargo test` 102 项全过;容器内交互待 Docker 环境复验)**:
  ...
  4. **验证与限制**:本机唯一服务器 192.168.79.150 **无 docker**(agent 实测 `docker: 未找到命令`,连 apt-get 都无——记录器完整捕获了弱模型"尝试安装 docker"的发散轨迹,ai_stop 兜住)。已验证:错误路径(无 docker 宿主上建 docker 会话连接→exec 请求发出→docker 报错文本进终端缓冲+通道 EOF→poller 心跳转 Disconnected,标签仍建)+ 输入路由(dock-test 会话敲命令→recorder 正确记录)。**✅ 已复验(见 P75)**:服务器装 docker(华为云镜像源拉 busybox)后,dock 会话真机连进 busybox 容器——`hostname` 返回容器主机名 helm-container、`ls /` 为容器精简 rootfs,交互命令实时工作(截图 `docs/screenshots/docker-terminal-live.png`)。
...
- [x] **P85 内置技能 Agent(用户提议,已完成,真机 6/8 任务命中技能 3s 确定性执行)**:
  1. **动机**:P84 基准暴露 glm4 病根——常见运维任务不需要 LLM 现场发挥(诊断类 56/71 步螺旋)。内置技能 = 意图关键词 + 精选命令序列(自带容错回退),命中即确定性执行完全绕过 LLM。
  2. **实现**:`skills.rs` 8 技能(磁盘/内存/日志/端口/sshd/用户/CPU/大文件)+**长度加权意图匹配**(字符数计分:"sshd"=4 压过 "报错"=2;`len()` 字节数对中文会错判 3 字节/字,已修)。ai_job Agent 分支开头命中技能→**复用全安全链执行**(计划卡/确认/task_exec/记录器),轨迹打 `source:"skill"` 金标=直接可训练的 SFT 样本;未命中走 LLM(零改动)。**训练闭环**:技能执行 = 确定性成功轨迹自动进记录器。
...
- [x] **P88 UI 三项优化(A/B/C 用户确认全做,已完成,真机截图 p88-agent-skill-clean.png 验证)**:
  1. **C 终端镜像精简**:`ai_echo_terminal` 配置(默认关)+设置 AI tab 勾选框「终端回显 AI 过程」;App `pushEcho` 加 force 参数门控——AI 命令回显/待确认/子目标标注默认不进终端(终端只留 shell 输出),连接失败 force=true 恒显示。真机:技能任务全程终端零 [AI] 行。
